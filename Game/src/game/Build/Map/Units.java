...
public class Units extends Component{
    ...
    private static boolean CanUnitMove(int terrain){
    ...
    private ArrayList<GameObject> Path = new ArrayList() ;
    ...
    public void cancelExecution_orders(){
        ...
        if(this.State == Executing_Orders){
            if( !this.time.isStart ){
                ...
                if(this.time.isEnd()){
                    if(this.move){
                        // my actual position
                        Vector2i myPos = this.gameObject.transform.position;
                        
                        // see if i can move of not because the terrain
                        if(CanUnitMove(terrain)){
                            // if i can move , get the next position of the path list
                            if(this.Path == null){
                                cancelExecution_orders();
                            }else{
                                if(this.Path.size() > 0){
                                    //    see which directions is the next movement
                                    Vector2i nxtPos = this.Path.get(0).getPosition();
                                    if(nxtPos.x > myPos.x && nxtPos.y == myPos.y){
                                        // Move Right
                                        this.gameObject.transform.position.x = nxtPos.x;
                                        this.gameObject.transform.position.y = nxtPos.y;
                                        this.Path.remove(0);

                                    }else if(nxtPos.x < myPos.x && nxtPos.y == myPos.y){
                                        // Move Left
                                        this.gameObject.transform.position.x = nxtPos.x;
                                        this.gameObject.transform.position.y = nxtPos.y;
                                        this.Path.remove(0);
                                    ...
                                    //if there is no more movements to do, or im in the target position.
                                    cancelExecution_orders();
                                }
                            }
                        }
...
//                      System.out.println("vs3:"+v.length());
    ...
    public void update(int GameState, GameObject temp , NodeWeighted nodew_target, NodeWeighted nodew_ActualPosition) {
        if(this.State == Waiting_for_Orders && this.unit_hasToken && GameState == 1){
            this.Path = Map.get().getShortestPath(nodew_target,nodew_ActualPosition);
            if (Path == null){
                cancelExecution_orders();
            }else{
